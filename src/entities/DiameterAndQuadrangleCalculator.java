...
public class DiameterAndQuadrangleCalculator  {
	...
	private void calculateQuadrangleSequence(QuadrangleSequence quadrangleSequence) {
		...
		if (!convexHull.isEmpty()) {
			...
			IHullIterator cIt = convexHull.rightIterator();

			// Convex hull has one point
			if (convexHull.hasOnePoint()) {
			...
			// Convex hull has more than two points
				...
				bIt = convexHull.leftIterator();
				dIt = convexHull.rightIterator();

				Quadrangle tmpQuadrangle;
				// limits for the iterators
				Point left = aIt.getPoint();
				Point right = cIt.getPoint();
				do {
	 ...
	 */
	private Quadrangle calculateQuadrangle(Point a, IHullIterator bIt, Point c, IHullIterator dIt) {
